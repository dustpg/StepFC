...
```
在CHR-ROM的长度为0时多分配8KB即可, 这里偷懒直接'或'了一下. 主要是因为C没有```std::max```, 又太懒╮（￣▽￣）╭
...
除了这两个还有通过硬件的IRQ(比如超级马里奥3利用Mapper触发IRQ, 状态栏在下面; 甚至通过APU的IRQ), 后面再说
...
### 同步
我们先实现一个简单的模式来模拟一帧(场), 也就是"中精度同步". 基于'同时只能显示25色'的假设(虽然可以在渲染时修改调色板以达到超过25色的可能, 也就是说这种情况是不支持的), 也就是, 和之前的实现差不多.
