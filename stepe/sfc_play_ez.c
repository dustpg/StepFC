...
/// <param name="ctx">The CTX.</param>
/// <param name="cps">The CPS.</param>
float sfc_n163_per_sample(sfc_famicom_t * famicom, sfc_n163_ctx_t * ctx, float cps) {
    ctx->clock += cps;
    ...
    int16_t output = 0; 
    assert(famicom->apu.n163.n163_lowest_id < 8);
    for (uint8_t i = famicom->apu.n163.n163_lowest_id; i != 8; ++i) {
        output += famicom->apu.n163.ch_output[i];
    }
    return (float)output;
}
...
void sfc_n163_samplemode_begin(sfc_famicom_t* famicom, sfc_n163_ctx_t* ctx) {
    ...
    ctx->count = (float)famicom->apu.n163.n163_count;
...
void sfc_n163_samplemode_end(sfc_famicom_t* famicom, sfc_n163_ctx_t* ctx) {
    famicom->apu.n163.n163_clock = (uint16_t)ctx->clock;
}
